...
function Show-FlashingWarning {
    ...
    Write-Host "`r$Text" -ForegroundColor Red  # Ensure final color is red after flashing
}
...
# Call the function directly instead of using Start-Job
Show-FlashingWarning "IMPORTANT: Press 'Q' when finished to properly exit and cleanup downloaded update files"


# Main Menu Loop
try {
    # Start flashing in a background job so it keeps running while menu is displayed
    $flashingJob = Start-Job -ScriptBlock {
        function Show-FlashingWarning {
            param (
                [string]$Text,
                [int]$Times = 99999,  # Make it keep flashing until user exits
                [int]$Delay = 500
            )
            for ($i = 0; $i -lt $Times; $i++) {
                Write-Host "`r$Text" -ForegroundColor Red -NoNewline
    ...
    # Cleanup job to prevent orphaned processes
    if ($flashingJob -and (Get-Job -Id $flashingJob.Id -ErrorAction SilentlyContinue)) {
        Stop-Job $flashingJob
        Remove-Job $flashingJob
    }
}
