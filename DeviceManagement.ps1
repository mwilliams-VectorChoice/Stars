...
function Show-MainMenu {
    ...
    function Show-FlashingWarning {
        ...
        while ($host.UI.RawUI.KeyAvailable -eq $false) {
            ...
            if ($stopwatch.Elapsed.TotalSeconds % 1 -lt 0.5) {
                $currentColor = $colors[0]
            } else {
                $currentColor = $colors[1]
            }

            # Position cursor to not interfere with menu
            $host.UI.RawUI.CursorPosition = $host.UI.RawUI.WindowSize.Width - 50, $host.UI.RawUI.WindowSize.Height - 2

            Write-Host "IMPORTANT: Press 'Q' when finished to properly exit and cleanup downloaded update files" -ForegroundColor $currentColor -NoNewline
    ...
    while ($true) {
        ...
        # Start flashing warning in background
        $job = Start-Job -ScriptBlock ${function:Show-FlashingWarning}

        # Wait for user input
        $mainChoice = Read-Host "Enter your choice"

        # Stop the flashing job
        Stop-Job $job
        Remove-Job $job
