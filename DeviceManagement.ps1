...
# Enhanced version to check if the current script is the latest version
$scriptVersion = "7.6.0"
$latestScriptUrl = "https://github.com/mwilliams-VectorChoice/Stars/releases/download/updated/DeviceManagement.ps1"
$localScriptPath = "$env:TEMP\DeviceManagement.ps1"

function Get-LatestScriptVersion {
    param ($url)
    try {
        $content = Invoke-WebRequest -Uri $url -UseBasicParsing -Method Head
        return $content.Headers["x-amz-meta-version"]
    } catch {
        Write-Warning "Failed to retrieve the latest version information."
        return $null
    }
}

$latestVersion = Get-LatestScriptVersion -url $latestScriptUrl

if ($latestVersion -and ($scriptVersion -ne $latestVersion)) {
    Write-Host "A new version ($latestVersion) is available. Downloading and running the latest version..." -ForegroundColor Yellow
    try {
        Invoke-WebRequest -Uri $latestScriptUrl -OutFile $localScriptPath -UseBasicParsing
        . $localScriptPath
        exit
    } catch {
        Write-Error "Failed to download or run the latest version of the script: $_"
    }
} else {
    Write-Host "You are running the latest version ($scriptVersion) of the script."
}
...
function Update-IntelGraphicsDriver {
    Write-Host "Checking Intel Graphics Driver..." -ForegroundColor Yellow
    
    # Get Intel Graphics Driver information
    $intelDriver = Get-WmiObject Win32_PnPSignedDriver | Where-Object {
        $_.DeviceName -like "*Intel(R) Iris* Xe*" -or 
        $_.DeviceName -like "*Intel(R) Arc*" -or 
        $_.DeviceName -like "*Intel(R) HD Graphics*"
    }

    if ($intelDriver) {
        $currentVersion = $intelDriver.DriverVersion
        $minVersion = [version]"32.0.101.6078"
        
        if ([version]$currentVersion -lt $minVersion) {
            Write-Host "Intel Graphics Driver needs updating (Current: $currentVersion)" -ForegroundColor Yellow
            
            # Download latest Intel Driver
            $driverUrl = "https://downloadmirror.intel.com/780555/gfx_win_101.6078.exe"
            $driverPath = "$env:TEMP\intel_graphics_driver.exe"
            
            try {
                Invoke-WebRequest -Uri $driverUrl -OutFile $driverPath
                Write-Host "Installing Intel Graphics Driver..." -ForegroundColor Yellow
                Start-Process -FilePath $driverPath -ArgumentList "-s -noreboot" -Wait
                Remove-Item $driverPath -Force
                Write-Host "Intel Graphics Driver updated successfully." -ForegroundColor Green
            }
            catch {
                Write-Host "Error updating Intel Graphics Driver: $_" -ForegroundColor Red
            }
        }
        else {
            Write-Host "Intel Graphics Driver is up to date (Version: $currentVersion)" -ForegroundColor Green
        }
    }
}

function Update-DotNetFramework {
    Write-Host "Checking .NET Framework installations..." -ForegroundColor Yellow
    ...
    # Install latest .NET Framework 4.8
    $net48Key = "HKLM:\SOFTWARE\Microsoft\NET Framework Setup\NDP\v4\Full"
    $net48Release = (Get-ItemProperty -Path $net48Key -Name Release -ErrorAction SilentlyContinue).Release
    
    if (-not $net48Release -or $net48Release -lt 528040) {
        Write-Host "Installing .NET Framework 4.8..." -ForegroundColor Yellow
        $net48Url = "https://go.microsoft.com/fwlink/?LinkId=2085155"
        $net48Installer = "$env:TEMP\net48.exe"
        
        try {
            Invoke-WebRequest -Uri $net48Url -OutFile $net48Installer
            Start-Process -FilePath $net48Installer -ArgumentList "/q /norestart" -Wait
            Remove-Item $net48Installer -Force
    ...
    Start-Process -FilePath "winget" -ArgumentList $processArgs -Wait -NoNewWindow
}

function Basic-Setup {
    # First update .NET Framework and Intel Graphics
    Update-DotNetFramework
    Update-IntelGraphicsDriver
...
#Disable Start Menu App Recommendations
function Disable-StartMenuRecommendations {
    Write-Host "Disabling Start Menu Recommendations..." -ForegroundColor Yellow
    ...
    foreach ($path in $registryValues.Keys) {
        foreach ($name in $registryValues[$path].Keys) {
            Set-ItemProperty -Path $path -Name $name -Value $registryValues[$path][$name] -Type DWord -Force
        }
    }

    # For user-specific settings, apply to all existing users
    $users = Get-ChildItem "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\ProfileList" | Where-Object { $_.PSChildName -match "S-1-5-21-" }
    foreach ($user in $users.PSChildName) {
        $userPath = "Registry::HKEY_USERS\$user\SOFTWARE\Microsoft\Windows\CurrentVersion\ContentDeliveryManager"
        if (Test-Path "Registry::HKEY_USERS\$user") {
            foreach ($name in $registryValues["HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\ContentDeliveryManager"].Keys) {
                if (!(Test-Path $userPath)) {
                    New-Item -Path $userPath -Force | Out-Null
                }
                Set-ItemProperty -Path $userPath -Name $name -Value $registryValues["HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\ContentDeliveryManager"][$name] -Type DWord -Force
            }
        }
    }

    Write-Host "Start Menu Recommendations disabled successfully" -ForegroundColor Green
}
...
function Remove-McAfeeRegistry {
    Write-Host "Performing deep McAfee registry cleanup..." -ForegroundColor Yellow
    
    # Define all possible McAfee service names
    $mcafeeServices = @(
        "mcafee*",
        "mfemms",
        "mfevtp",
    ...
    foreach ($service in $mcafeeServices) {
        Get-Service -Name $service -ErrorAction SilentlyContinue | ForEach-Object {
            ...
            $result = Start-Process -FilePath "sc.exe" -ArgumentList "delete", $_.Name -Wait -PassThru -WindowStyle Hidden
            if ($result.ExitCode -eq 0) {
                Write-Host "Removed service: $($_.Name)" -ForegroundColor Green
            }
        }
    }
        ...
        foreach ($key in $keys) {
            ...
            if ($displayName -like "*McAfee*") {
                try {
                    ...
                    Write-Host "Removed registry key for: $displayName" -ForegroundColor Green
                }
                catch {
                    Write-Host "Failed to remove registry key for: $displayName" -ForegroundColor Red
                }
            }
        }
    }

    # Additional McAfee registry locations
    $additionalPaths = @(
    ...
    foreach ($path in $additionalPaths) {
        if (Test-Path $path) {
            try {
                Remove-Item -Path $path -Recurse -Force -ErrorAction SilentlyContinue
                Write-Host "Removed additional McAfee registry path: $path" -ForegroundColor Green
            }
            catch {
                Write-Host "Failed to remove registry path: $path" -ForegroundColor Red
            }
        }
    }

    # Clean Windows Installer phantom entries
    $installerKeys = "HKLM:\SOFTWARE\Classes\Installer\Products"
    if (Test-Path $installerKeys) {
        Get-ChildItem $installerKeys | ForEach-Object {
            $key = $_
            $productName = (Get-ItemProperty -Path "$($key.PSPath)\SourceList" -Name "PackageName" -ErrorAction SilentlyContinue).PackageName
            if ($productName -like "*McAfee*") {
                Remove-Item -Path $key.PSPath -Recurse -Force -ErrorAction SilentlyContinue
                Write-Host "Removed Windows Installer phantom entry: $productName" -ForegroundColor Green
            }
        }
    }
    ...
    $startMenuPaths = @(
        ...
        "$env:ALLUSERSPROFILE\Microsoft\Windows\Start Menu\Programs\McAfee"
    )
    foreach ($path in $startMenuPaths) {
        if (Test-Path $path) {
            Remove-Item -Path $path -Recurse -Force -ErrorAction SilentlyContinue
            Write-Host "Removed Start Menu entries from: $path" -ForegroundColor Green
        }
    }
    ...
    foreach ($path in $filePaths) {
        if (Test-Path $path) {
            try {
                Remove-Item -Path $path -Recurse -Force -ErrorAction SilentlyContinue
                Write-Host "Removed McAfee files from: $path" -ForegroundColor Green
            }
            catch {
                Write-Host "Failed to remove files from: $path" -ForegroundColor Red
            }
        }
    }

    # Force refresh Windows Installer cache
    try {
        ...
        Write-Host "Refreshed Windows Installer cache" -ForegroundColor Green
    }
    catch {
        Write-Host "Failed to refresh Windows Installer cache" -ForegroundColor Red
    }

    # Final cleanup of any remaining phantom entries
    $phantomKey = "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Installer\UserData\S-1-5-18\Products"
    if (Test-Path $phantomKey) {
        Get-ChildItem $phantomKey | ForEach-Object {
            $key = $_
            $displayName = (Get-ItemProperty -Path "$($key.PSPath)\InstallProperties" -Name "DisplayName" -ErrorAction SilentlyContinue).DisplayName
            if ($displayName -like "*McAfee*") {
                Remove-Item -Path $key.PSPath -Recurse -Force -ErrorAction SilentlyContinue
                Write-Host "Removed phantom installer entry: $displayName" -ForegroundColor Green
            }
        }
    }
}

function Remove-McAfeeCompletely {
    ...
    if ($mcafeeinstalled -eq "true") {
        ...
        # Download and run MCPR.exe (McAfee Consumer Product Removal Tool)
        ...
        # Download and run newer version of MCPR
        ...
        # Find and remove all McAfee programs
        ...
        # Remove leftover Start Menu items and registry keys
