...
$host.UI.RawUI.WindowSize = New-Object System.Management.Automation.Host.Size(($host.UI.RawUI.MaxWindowSize.Width), ($host.UI.RawUI.MaxWindowSize.Height))
$host.UI.RawUI.BufferSize = New-Object System.Management.Automation.Host.Size(($host.UI.RawUI.MaxWindowSize.Width), 9999)

try {
    $window = (Get-Process -Id $pid).MainWindowHandle
    if ($window) {
        $null = [Console]::ShowWindow($window, 3)
    }
} catch {}
...
function Show-MainMenu {
    ...
    Write-Host "| ( (_)| |__     *   (*)   ___    __   " -ForegroundColor DarkMagenta
    Write-Host "| |  * |  * `\ /'_`\ | | /'___) /'__`\ " -ForegroundColor Magenta
    ...
    Write-Host "7. System Information" -ForegroundColor Cyan
...
function Show-SystemInfo {
    ...
    $drives = Get-CimInstance CIM_LogicalDisk | Where-Object {$_.DriveType -eq 3}
    $network = Get-CimInstance CIM_NetworkAdapter | Where-Object {$_.PhysicalAdapter -eq $true}
    ...
    $networkAdapters = Get-NetAdapter | Where-Object { $_.Status -eq "Up" } | ForEach-Object {
        ...
        [PSCustomObject]@{
            Name = $_.Name
            IPAddress = ($addresses | Where-Object { $_.AddressFamily -eq 'IPv4' }).IPAddress
            MACAddress = $_.MacAddress
            Speed = [math]::Round($_.LinkSpeed/1000000000, 2)  # Convert to Gbps
        }
    }
    ...
    # Memory
    $totalMemory = [math]::Round(($memory | Measure-Object -Property Capacity -Sum).Sum / 1GB, 2)
    ...
    Write-Host "Free Memory: $([math]::Round($totalMemory - $usedMemory, 2)) GB"
    
    # Antivirus Information
    Write-Host "`nAntivirus Status:" -ForegroundColor Yellow
    if ($antiVirus) {
        foreach ($av in $antiVirus) {
            Write-Host "Product: $($av.displayName)"
            Write-Host "Status: $($av.productState)"
        }
    } else {
    ...
    foreach ($adapter in $networkAdapters) {
        ...
        Write-Host "  Link Speed: $($adapter.Speed) Gbps"
    }
    ...
    Write-Host "`nPress any key to return to the main menu..." -ForegroundColor Green
    $null = $Host.UI.RawUI.ReadKey('NoEcho,IncludeKeyDown')
}
