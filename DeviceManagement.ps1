...
# Add this near the top of your script, after initializing $script:warningColor
$script:warningColorIndex = 0
$script:warningColors = @('Red', 'Yellow')
...
function Show-MainMenu {
    ...
    # Alternate colors for the warning message
    if ($script:warningColor -eq 'Red') {
        $script:warningColor = 'Yellow'
    } else {
        $script:warningColor = 'Red'
    }
    
# Debug: Always print out current color
    Write-Host "Current Warning Color Index: $script:warningColorIndex" -ForegroundColor Green
    Write-Host "Current Warning Color: $($script:warningColors[$script:warningColorIndex])" -ForegroundColor Green

    # Increment the color index, wrapping around to 0 if it reaches the end
    $script:warningColorIndex = ($script:warningColorIndex + 1) % $script:warningColors.Count
    
    Write-Host "IMPORTANT: Press 'Q' when finished to properly exit and cleanup downloaded update files" -ForegroundColor $script:warningColors[$script:warningColorIndex]
    Write-Host "If you exit any other way, temporary files will remain on your system" -ForegroundColor $script:warningColors[$script:warningColorIndex]
}
