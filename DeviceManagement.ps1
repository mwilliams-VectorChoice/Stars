...
# Function to set the PowerShell window style
function Set-WindowStyle {
    param (
        [ValidateSet("Hide", "Minimize", "Maximize", "Restore", "Show")]
        [string]$Style
    )

    # Get the current PowerShell window
    $hwnd = (Get-Process -Id $PID).MainWindowHandle
    $ShowWindowAsync = Add-Type @"
        using System;
...
"@ -PassThru
    ...
    switch ($Style) {
        "Hide" { $ShowWindowAsync::ShowWindowAsync($hwnd, 0) | Out-Null }
        "Minimize" { $ShowWindowAsync::ShowWindowAsync($hwnd, 2) | Out-Null }
        "Maximize" { $ShowWindowAsync::ShowWindowAsync($hwnd, 3) | Out-Null }
        "Restore" { $ShowWindowAsync::ShowWindowAsync($hwnd, 9) | Out-Null }
        "Show" { $ShowWindowAsync::ShowWindowAsync($hwnd, 5) | Out-Null }
    }
}
