...
function Show-SystemInfo {
    ...
    $networkAdapters = Get-NetAdapter | Where-Object { $_.Status -eq "Up" } | ForEach-Object {
        ...
        # Extract numeric value from LinkSpeed
        $speedValue = if ($_.LinkSpeed -match '(\d+)') {
            [double]$matches[1]
        } else {
            0
        }
        
        # Determine the unit and convert to Gbps
        $speedInGbps = if ($_.LinkSpeed -like "*Gbps") {
            $speedValue
        } elseif ($_.LinkSpeed -like "*Mbps") {
            $speedValue / 1000
        } elseif ($_.LinkSpeed -like "*Kbps") {
            $speedValue / 1000000
        } else {
            0
        }
        ...
        [PSCustomObject]@{
            ...
            Speed = [math]::Round($speedInGbps, 2)
        }
    }
