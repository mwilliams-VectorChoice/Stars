...
try {
    while ($true) {
        $mainChoice = Read-Host "`nEnter your choice"
        ...
        # Add other menu choices handling here
    }
} finally {
    # Ensure the job is cleaned up if the script exits unexpectedly
    $job = Get-Job -Id $flashingJob.Id -ErrorAction SilentlyContinue
    if ($job) {
        Stop-Job $job
        Remove-Job $job
    }
